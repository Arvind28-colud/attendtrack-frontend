...
export default function Register() {
  ...
  const handleAadhaarUpload = async () => {
    ...
    try {
      // Upload PDF to Cloudinary, store URL in DB
      setAadhaarAlert({ type:"info", msg:"Uploading to cloud..." });
      const b64 = await fileToBase64(aadhaarFile);
      const pdfUrl = await uploadPdf(b64, `aadhaar_${newEmpId}_${Date.now()}`);
      await api.updateAadhaarPdf(newEmpId, pdfUrl);
