  ...
  return (
    <div>
      <div className="metrics-grid">
        <div className="metric metric-dark"><div className="metric-label">Total Employees</div><div className="metric-value">{data.total_employees}</div><div className="metric-sub">registered</div></div>
        <div className="metric metric-dark"><div className="metric-label">Present Today</div><div className="metric-value">{data.present}</div><div className="metric-sub">clocked in</div></div>
        <div className="metric metric-light"><div className="metric-label">Absent Today</div><div className="metric-value">{data.absent}</div><div className="metric-sub">not yet in</div></div>
        <div className="metric metric-light"><div className="metric-label">OT Hours</div><div className="metric-value">{data.ot_hours}h</div><div className="metric-sub">today</div></div>
      </div>
      <div className="grid2">
        <div className="card">
          <div className="card-title">Currently Working</div>
          {data.on_duty.length===0 ? <p className="muted">No one currently working.</p>
            : data.on_duty.map(e=>(
        ...
        <div className="card">
          ...
          {data.absent_employees.length===0 ? <p className="muted">All employees are in!</p>
              ...
              {todayAtt.length===0
                ...
                : todayAtt.map((r,i)=>(
                  <tr key={i}>
                    ...
                    <td><span className={`badge badge-${r.status==="present"||r.status==="on-duty"?"in":r.status==="on-duty"?"work":"absent"}`}>{r.status==="on-duty"?"Working":r.status}</span></td>
                  </tr>
                ))}
            </tbody>
          </table>
