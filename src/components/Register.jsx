...
export default function Register() {
  ...
  const handleAadhaarUpload = async () => {
    ...
    try {
      // Save PDF locally via backend
      setAadhaarAlert({ type: "info", msg: "Uploading Aadhaar PDF..." });
      const b64 = await fileToBase64(aadhaarFile);
      const pdfDataUri = `data:application/pdf;base64,${b64}`;
      await api.updateAadhaarPdf(newEmpId, pdfDataUri);
          ...
          {step === 1 && (
            <>
              ...
              {form.source && form.source !== "TTIPL" && (
                <div style={{
                  borderTop: "1px solid var(--border)", marginTop: "1rem",
